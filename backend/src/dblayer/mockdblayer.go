...
func NewMockDBLayer(products []models.Product, customer []models.Customer, orders []models.Order) *MockDBLayer {
	return &MockDBLayer{
		products:  products,
		customers: customer,
		orders:    orders,
...
func (mock *MockDBLayer) GetCustomerById(id int) (models.Customer, error) {
	...
	return result, fmt.Errorf("No customer with id: %d", id)
}

func (mock *MockDBLayer) GetProduct(id uint) (models.Product, error) {
	result := models.Product{}
	if mock.err != nil {
		return result, mock.err
	}
	for _, customer := range mock.products {
		if customer.ID == uint(id) {
			return customer, nil
		}
	}
...
func (mock *MockDBLayer) GetCustomerOrdersById(id int) ([]models.Order,
	error) {
	if mock.err != nil {
		return nil, mock.err
	}
...
func (mock *MockDBLayer) AddOrder(order models.Order) error {
	...
	return fmt.Errorf("Could not find customer id %d for order",
		order.CustomerID)
}
